...
**Push with `clasp-cc`** (CC Apps Script account — `clasp -A ~/.clasp/cc.clasprc.json`). Non-interactive tools use the repo wrapper **[`scripts/clasp-cc`](./scripts/clasp-cc)** (same as the alias). Web redeploy helper: **[`scripts/redeploy-web-apps.sh`](./scripts/redeploy-web-apps.sh)**.
